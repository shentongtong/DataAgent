...
@Component
@AllArgsConstructor
public class JavaAnalyzeNode implements NodeAction {
	...
	public Map<String, Object> apply(OverAllState state) throws Exception {
		...
		String javaOutput = StateUtil.getStringValue(state, JAVA_EXECUTE_NODE_OUTPUT);
		...
		String systemPrompt = PromptConstant.getJavaAnalyzePromptTemplate()
			.render(Map.of("java_output", javaOutput, "user_query", userQuery));

		Flux<ChatResponse> javaAnalyzeFlux = llmService.callSystem(systemPrompt);

		Flux<GraphResponse<StreamingOutput>> generator = FluxUtil.createStreamingGeneratorWithMessages(this.getClass(),
				state, "正在分析代码运行结果...\n", "\n结果分析完成。", aiResponse -> {
					Map<String, String> updatedSqlResult = PlanProcessUtil.addStepResult(sqlExecuteResult, currentStep,
							aiResponse);
					log.info("java analyze result: {}", aiResponse);
					return Map.of(SQL_EXECUTE_NODE_OUTPUT, updatedSqlResult, PLAN_CURRENT_STEP, currentStep + 1);
				}, javaAnalyzeFlux);

		return Map.of(JAVA_ANALYSIS_NODE_OUTPUT, generator);
	}
